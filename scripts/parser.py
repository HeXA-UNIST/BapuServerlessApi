...
기숙사 식당 식단표에서 조식, 중식, 석식으로 구분된 메뉴의 경우 restaurant_name을 기숙사_식당_한식으로 출력해줘.
만약 왼쪽에 있는 구분이 할랄일 경우 restaurant_name을 기숙사_식당_할랄이 되도록 해줘.
이는 중요한 요소로 위반할 경우 모든 포인트를 잃어.

너는 반드시 주어진 사진에 있는 모든 식단을 출력해야해.
기숙사 식당의 경우 반드시 LUNCH가 2개여야해(한식, 할랄).
"""
